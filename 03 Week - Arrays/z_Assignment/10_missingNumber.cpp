...
#include<vector>
using namespace std ; 
// similar question in assignment , finding the duplicate
...
int main(){
    ...
    for(int i = 1 ; i < v.size() ; i++){
        ...
        if(v[index] > 0){
            //! - this condition is important if the element is already negative don't make it negative and if it is positive so make it negative it means we have visited the element  
            v[index] = v[index] * -1 ; 
        }
    }
